...
from experiment_project.utils.files.dir import make_dir
from experiment_project.utils.model.load import load_embedding_model_by_sentence_transformer
...
df = pd.read_csv('/mnt/d/project/zzbc/experiment_project/experiment_project/audio/data/audio/product_list_ez_cloud.csv')
faiss_obj = FaissIndex(model_path='/mnt/d/models/embeddings/jinaai/jina-embeddings-v2-base-zh/',
                       faiss_dir_path='/mnt/d/project/zzbc/experiment_project/tests/data')
df_data = df.drop(['Unnamed: 0'],axis=1).to_dict('records')
faiss_obj.add(documents=[json.dumps(doc) for doc in df_data[0:1]])


topK = 5
search = faiss_obj.model.encode(["服务端"])
D, I = faiss_obj.index.search(search, topK)
df['ERP型号'].iloc[I[0]]
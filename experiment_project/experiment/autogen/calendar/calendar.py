...
### Restrictions (限制)
所有操作必须在用户授权的范围内进行，确保数据的安全和隐私。对于API接口的调用，需要遵守API文档中的格式和协议。
...
### Restrictions (限制)
确保所有建议都基于用户提供的数据和明确的偏好，避免提供泛泛而谈的建议。同时，保护用户隐私，不泄露具体的日程细节。
...
Restrictions (限制): 
确保所有的集成和数据交换操作都符合最高的安全标准，并且能够有效地处理错误和异常情况。
...
**Restrictions (限制)**: 
设计时需考虑系统的可扩展性、兼容性和用户界面的易用性，确保安全性和数据保护符合最新标准。
...
functions_dict = load_functions_from_directory('/mnt/d/project/zzbc/experiment_project/experiment_project/experiment/autogen/calendar/tools')
...
for func_name,func in functions_dict.items():
    for agent in agent_list:
        if agent.name.replace('_agent','') in func_name:
            agent.register_for_llm(name=func_name, description="A simple calculator")(func)

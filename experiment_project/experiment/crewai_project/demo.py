...
verbose=True,
allow_delegation=False,llm=OpenAIGPT4,tools=tools,max_iter=max_iter)

# Define the blog writer agent
intelligent_recommendation_agent = Agent(
role="用户希望通过智能建议Agent获取个性化的时间+管理和日程优化建议",
...
explainSnippet = Task(
description=f"帮我查询2024年5月15日到2024年5月17日所有的安排",
agent=calendar_event_agent,
expected_output="输出为一个表格"
)
writeBlogPost = Task(
description=f"统计我在各个事件上面的事件占用比,根据不同的事务和事件对我进行提醒",
agent=calendar_event_agent,
expected_output="",
    human_input=True
)
...
remove_meeting = Task(
description=f"帮我去除掉2024年6月1日所有的安排,我要在家好好休息一天",
agent=meeting_agent,
expected_output="查看2024年6月1日的安排",
human_input=True
)
...
crew = Crew(
agents=[calendar_event_agent, intelligent_recommendation_agent,meeting_agent],
tasks=[explainSnippet, writeBlogPost,remove_meeting],
# tasks=[explainSnippet,task_a],
verbose=2,
